...
RhoMax_vec = {'RhoMax=10',...
              'RhoMax=10^3',...
              'RhoMax=10^6',...
              'RhoMax=10^8',...
              };

ckappa_ests = cell(length(Triangulierung_vec),length(RhoMax_vec));
...
%         tiledlayout('flow')
        ...
        [cu,u_FETIDP_glob,~,~,ckappa_ests{i}{r}] = fetidp_constraint(TOL,vert__sd,tri__sd,l2g__sd,f,...
...
fprintf('Konditionszahl fuer unterschiedliche RhoMax und Triangulierungen: \n')
T_results = cell2table([ckappa_ests{1};ckappa_ests{2};ckappa_ests{3}],"RowNames",rowNames,"VariableNames",RhoMax_vec);
disp(T_results)