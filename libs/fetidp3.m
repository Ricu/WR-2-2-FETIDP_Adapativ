function [cu,u_FETIDP_glob] = fetidp3(numSD,vert,numVert,vert__sd,tri__sd,edges,numEdges,l2g__sd,f,dirichlet,VK,maxRhoSD,maxRhoVert,plot)
...
for i = 1:numEdgesSD
    % extrahiere Knotenindizes aus Kante
    ind = kantenliste(i);
    % Stelle pi_bar_ij, pi_ij, P_D_ij, S_ij, I auf
    % Loese eigenwertproblem auf kante
    sigma = 1;
    [eigenvalues, eigenvectors] = adaptiveEigenvalues(pi_bar_e, pi_e, P_D_e, S_e,sigma);
    eigenvectors = eigenvectors(:,eigenvalues > TOL);

    % Extrahiere u
    n_EV = size(eigenvectors,2);
    U_temp = zeros(n_LM,n_EV);
    for k = 1:n_EV
        U_temp(ind,k) = B_D_edge * S_edge * P_D_edge * eigenvectors(:,K_PiPiTilde);
    end
    U = [U , U_temp];
end
...
for i=1:numSD
 ...
 %% NEUE
function [eigenvalues, eigenvectors] = adaptiveEigenvalues(pi_bar, pi, P_D, S,sigma)
    LHS = pi_bar * pi * P_D' * S * P_D * pi * pi_bar;
    temp = pi * S * pi + sigma(eye(size(pi))-pi);
    RHS = pi_bar * temp * pi_bar + sigma(eye(size(pi_bar))-pi_bar);
    [eigenvalues, eigenvectors] = eigen(RHS,LHS);
%         [eigenvalues, eigenvectors] = eigen(RHS\LHS);
end